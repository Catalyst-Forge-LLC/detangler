...
- **Site:** FilePress (`getfilepress`) + Cloudflare Pages (`pnpm ship`, project `detangler`). LocalSlip lease `detangler-site` on **5203**. `pnpm site:dev` runs `localslip claim`, then FilePress reads the lease.
