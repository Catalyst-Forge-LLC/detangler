...
| Argument | Section questions and a progression basis. `no` is not automatically broken. A claim inside a node that never joined the weave (`unwoven_claim`) or leftover from another job (`debris`) | Journeys that start and never resolve; a form that collects data no feature consumes; onboarding that promises a capability the app does not have; copy that names a capability with no contract; leftover from a removed screen |
| Continuity | Passage-to-passage development: backtrack, delayed payoff, missing handoff, fragmented development. Drafts only | — |
...
| Strands (refuse) | Line editing, tone, rhythm. A continuity action may name a one-sentence repair. The report does not include a rewritten sentence. | Lint, naming, “clean up this file,” implementation style, component internals, performance, security |
...
The outline at the bottom of the report is load-bearing. Findings already carry `node_id`. Keep it that way. A continuity finding still names a node. Apply agents that rewrite sentences beyond the approved relationship have left the hour.
